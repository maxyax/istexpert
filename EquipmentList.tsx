...
import { Search, Plus, Truck, X, QrCode, Zap, Activity, Save, Edit3, Camera, FileText, Clock } from 'lucide-react';
...
export const EquipmentList: React.FC = () => {
  const { equipment, selectEquipment, selectedEquipmentId, updateEquipment, updateRegulations } = useFleetStore();
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-4">
          ...
          <button className="p-3 rounded-2xl bg-blue-500 text-white shadow-lg hover:shadow-neo transition-all"><Plus size={24} /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map(e => (
          <div key={e.id} onClick={() => selectEquipment(e.id)} className="p-8 rounded-[2.5rem] shadow-neo bg-neo-bg group hover:shadow-neo-inset transition-all cursor-pointer border border-white/10">
            ...
            <p className="text-[10px] font-black text-blue-500 uppercase mt-1 tracking-widest">{e.make} {e.model}</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
          <div className="bg-neo-bg w-full max-w-5xl h-[90vh] rounded-[3rem] shadow-neo overflow-hidden flex flex-col border border-white/20 animate-in zoom-in duration-300">
            <div className="p-8 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white/5">
              ...
              <div className="flex gap-4">
                <button onClick={handleGenerateAI} className="p-4 rounded-2xl shadow-neo text-blue-500 hover:shadow-neo-inset transition-all"><Zap size={20}/></button>
