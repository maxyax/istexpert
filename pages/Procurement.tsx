                   ...
                   <div className="p-4 md:p-6 rounded-2xl shadow-neo bg-neo-bg border border-white/5">
                     ...
                     <div className="space-y-2">
                       ...
                       {(editReq.items || []).map((it: any, idx: number) => (
                         <div key={it.id || idx} className="grid grid-cols-12 gap-2 items-center bg-white/5 p-2 rounded-xl">
                           <button className="col-span-1 text-red-500 font-bold text-lg hover:text-red-600 text-center" onClick={() => { const arr = [...editReq.items]; arr.splice(idx,1); setEditReq({...editReq, items: arr}); }}>×</button>
                           <input className="col-span-2 md:col-span-2 p-2 rounded-xl shadow-neo-inset bg-neo-bg border-none app-input text-xs" placeholder="Арт" value={it.sku || ''} onChange={e=>{ const arr = [...editReq.items]; arr[idx].sku = e.target.value; setEditReq({...editReq, items: arr}); }} />
                           <input className="col-span-9 md:col-span-5 p-2 rounded-xl shadow-neo-inset bg-neo-bg border-none app-input text-xs" placeholder="Наименование" value={it.name} onChange={e=>{ const arr = [...editReq.items]; arr[idx].name = e.target.value; setEditReq({...editReq, items: arr}); }} />
                           ...
                           <div className="col-span-2 md:col-span-1 text-right text-xs font-black text-emerald-600">{formatMoney(it.total || 0)}</div>
                         </div>
                       ))}
