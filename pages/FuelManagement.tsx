       ...
       <div className="bg-neo-bg rounded-[3rem] shadow-neo-inset p-10 overflow-hidden border border-white/5 relative">
          ...
          <div className="space-y-6 max-h-[550px] overflow-y-auto custom-scrollbar pr-4">
            {fuelRecords.length === 0 ? (
              ...
              <div key={r.id} className="p-8 rounded-[2.5rem] shadow-neo bg-neo-bg flex justify-between items-center border-l-4 border-blue-500 group hover:shadow-neo-inset transition-all cursor-default border border-white/5">
                <div className="flex items-center gap-8">
                   <div className="p-4 rounded-2xl shadow-neo bg-neo-bg text-blue-500 group-hover:scale-110 transition-transform"><Fuel size={28}/></div>
                   <div>
                      <p className="text-sm font-black uppercase tracking-tight text-gray-800 dark:text-gray-100">{equipment.find(e=>e.id===r.equipmentId)?.name || 'Неизвестно'}</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{r.station} • {new Date(r.date).toLocaleDateString('ru-RU')}</p>
                   </div>
                </div>
                <div className="text-right">
                   <p className="text-2xl font-black text-blue-500">+{r.quantity} л</p>
                   <p className="text-[11px] font-black text-emerald-500 tracking-widest">{r.totalCost.toLocaleString()} ₽</p>
                </div>
              </div>
            ))}
          </div>
